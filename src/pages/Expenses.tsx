...
import { Plus, ArrowDownLeft, Search, Filter, MoreVertical, Pencil, Trash2, Calendar as CalendarIcon, ArrowRight, Zap, Target, Shapes, CreditCard } from 'lucide-react';
...
export const Expenses: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(editingTransaction ? 'Resource allocation adjusted' : 'Capital outflow recorded');
  ...
  const handleDelete = async (t: Transaction) => {
    if (!user || !confirm('Permanently nullify this capital outflow record?')) return;
    try {
      await runTransaction(db, async (transaction) => {
  ...
  return (
    <div className="space-y-10 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        >
          <h2 className="text-3xl font-black tracking-tight text-slate-900 dark:text-white uppercase italic">Capital Outflow</h2>
          <p className="text-sm text-slate-500 font-medium">Coordinate and track your resource depletion</p>
        </motion.div>
        ...
        <Dialog open={isDialogOpen} onOpenChange={(open) => {
          ...
          <DialogTrigger render={
            <Button className="h-12 rounded-2xl font-black px-8 shadow-lg shadow-rose-500/20 gap-2 bg-rose-500 hover:bg-rose-600 text-white uppercase tracking-tight italic">
              <Plus className="h-5 w-5" /> Record Outflow
            </Button>
          } />
          <DialogContent className="sm:max-w-xl rounded-[32px] border-none shadow-2xl p-0 overflow-hidden">
            <div className="bg-rose-500 p-8 text-white relative overflow-hidden">
               ...
               <DialogTitle className="text-3xl font-black uppercase italic tracking-tight">Outflow Protocol</DialogTitle>
               <p className="text-rose-100/70 text-sm font-bold italic">Initialize a new capital depletion event</p>
            </div>
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Consumption Magnitude</Label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-rose-500">{profile?.currency === 'PKR' ? 'Rs' : '$'}</span>
                ...
                <div className="space-y-2">
                   <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Event Classification</Label>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Protocol Alias (Title)</Label>
                <Input className="h-14 rounded-2xl font-black bg-slate-50 dark:bg-slate-900 border-none px-6" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Energy Grid Payment" required />
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                   <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Source Node</Label>
              ...
              <DialogFooter className="pt-4">
                <Button type="submit" className="w-full h-16 rounded-2xl text-lg font-black bg-rose-500 hover:bg-rose-600 text-white group" disabled={loading || !accountId}>
                  {loading ? 'Transmitting...' : editingTransaction ? 'Calibrate record' : 'Initialize Outflow'}
      ...
      <Card className="border-none shadow-2xl shadow-slate-200/50 dark:shadow-none dark:border dark:border-slate-800 rounded-[40px] overflow-hidden bg-white dark:bg-slate-900">
        <CardHeader className="p-8 pb-4">
          <div className="relative flex-1 max-w-md group">
            ...
            <Input 
              placeholder="Filter capital depletion logs..." 
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 italic">Identified Operation</TableHead>
                  ...
                  {filteredTransactions.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="py-24 text-center">
                        ...
                        <p className="text-slate-400 font-black uppercase italic tracking-widest px-8">No matching capital depletion records found within the audit spectrum.</p>
                      </TableCell>
                    </TableRow>
                  ) : (
                      ...
                      >
                        <TableCell className="px-8 py-6">
                          <div className="flex items-center gap-4">
                            <div className="h-12 w-12 rounded-2xl bg-rose-50 dark:bg-rose-500/10 flex items-center justify-center text-rose-500 border border-rose-100 dark:border-rose-500/20 group-hover:rotate-6 transition-transform">
                        ...
                        <TableCell className="px-8 py-6 text-2xl font-black text-right text-rose-500 tabular-nums tracking-tighter italic">
